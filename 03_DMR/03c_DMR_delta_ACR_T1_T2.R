...
# Remove missing delta ACR values
targets <- targets[!is.na(targets$delta_LogACR), ]
...
############################
# Bumphunter (cutoff 0.00035)
############################

tab <- bumphunter(GMset_culled, design, coef = 2, cutoff = 0.00035)
...
write.table(
  dmr_out,
  "DMRS_deltaACR_T1_cutoff0.00035_boot500.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
