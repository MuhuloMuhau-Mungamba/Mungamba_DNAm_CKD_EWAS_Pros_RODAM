...
#
# EWAS of change in albuminuria (ΔACR, log scale)
# - Baseline (T1)
# - Follow-up (T2)
#
# Study design:
...
#
# Model:
# Δlog(ACR) ~ age + sex + site + cell counts + plate
#
# Notes:
...
targets <- targets %>%
  filter(!is.na(delta_LogACR))

############################
# Define variables
############################

delta_ACR <- targets$delta_LogACR
...
design_base <- model.matrix(~ delta_ACR)
...
write.table(
  final_T1,
  "CKD_EWAS_T1_delta_ACR_results.txt",
  sep = "\t",
  quote = FALSE,
  row.names = TRUE,
  col.names = TRUE
...
write.table(
  final_T2,
  "CKD_EWAS_T2_delta_ACR_results.txt",
  sep = "\t",
  quote = FALSE,
  row.names = TRUE,
  col.names = TRUE
