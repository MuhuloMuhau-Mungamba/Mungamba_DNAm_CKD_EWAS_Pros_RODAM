############################################
# CKD EWAS – Sensitivity Analysis (ΔeGFR)
# 07b_Sensitivity_EWAS_Delta_eGFR_T1_T2.R
#
# Author: Muhulo Muhau Mungamba & Andrea Venema
# Date: 2025-09-26
#
# Description:
# Sensitivity EWAS for ΔeGFR (continuous outcome)
# Adjusted for:
...
out_T1   <- "2025.09.26_Sensitivity_EWAS_Delta_eGFR_T1.txt"
...
top_m <- topTable(fit, coef = "deltaegfr", number = nrow(mval), adjust = "fdr")

############################
# Bacon (T1)
############################

tstats <- fit$t[, "deltaegfr"]
...
top_b <- topTable(fit_b, coef = "deltaegfr", number = nrow(beta), adjust = "fdr")
...
out_T2   <- "2025.09.26_Sensitivity_EWAS_Delta_eGFR_T2.txt"

targets <- read.csv(pheno_T2)

############################
# Repeat preprocessing (T2)
############################
